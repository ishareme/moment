<template>
    <div class="film-list">
        ...
        <div class="input-search">
            <div class="main">
                <input type="text" v-focus="focused" @focus="searchFocus" @blur="searchBlur" v-model="searchTxt" placeholder="Search Article" ref="filmSearchInput">
                <div class="search-icon" @click="searchFilmArticle"></div>
            </div>
        </div>
        <scroller class="scroller" ref="filmScroller"
                  :on-refresh="refresh"
        ...
        >
            ...
            <div class="container" id="scrollFilmList">
                <div class="banner" v-for="(item, index) in filmArticleList" v-bind:key="item._id" @click="toArticleDetail(item)">
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.$axios.getFilmArticleList({current: this.current, isWebApp: true}).then(res => {
                ...
                this.preload(this.filmArticleList.map(item => {
        ...
        watch: {
            searchTxt(newVal){
                this.$axios.getFilmArticleList({current: this.current, searchKey: newVal,  isWebApp: true}).then((res) => {
                    this.filmArticleList = res.data.data.docs
                    console.log('this.filmArticleList',this.filmArticleList)
                })
            }
        ...
        methods: {
            ...
            searchFilmArticle(){
                this.$axios.getFilmArticleList({current: this.current, searchKey: this.searchTxt,  isWebApp: true}).then((res) => {
                    this.filmArticleList = res.data.data.docs
                    console.log('this.filmArticleList',this.filmArticleList)
                })
            }
